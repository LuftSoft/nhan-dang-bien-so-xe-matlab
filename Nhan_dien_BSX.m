...
% NHAN_DIEN_BSX MATLAB code for Nhan_dien_BSX.fig
%      NHAN_DIEN_BSX, by itself, creates a new NHAN_DIEN_BSX or raises the existing
%      singleton*.
%
%      H = NHAN_DIEN_BSX returns the handle to a new NHAN_DIEN_BSX or the handle to
%      the existing singleton*.
%
%      NHAN_DIEN_BSX('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NHAN_DIEN_BSX.M with the given input arguments.
%
%      NHAN_DIEN_BSX('Property','Value',...) creates a new NHAN_DIEN_BSX or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Nhan_dien_BSX_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Nhan_dien_BSX_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Nhan_dien_BSX

% Last Modified by GUIDE v2.5 17-Oct-2022 12:07:27
...
try 
    ...
    disp(report(e))
    msgbox("Ban chua mo camera","Error")
end
...
cla(handles.duoi1,'reset');cla(handles.duoi2,'reset');cla(handles.duoi3,'reset');cla(handles.duoi4,'reset');cla(handles.duoi5,'reset');
